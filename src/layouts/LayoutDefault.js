...
export const scrollTop = (top) => {
  ...
  //const val = MathUtil.mapValueInRange(top, 0, scrollHeight, scrollHeight * 0.0, scrollHeight * 1.0 - window.innerHeight);
  //console.log(val + " " + scrollHeight);
  spring.setCurrentValue(scrollTop).setAtRest();
  spring.setEndValue(val );

  const test = () => {
    if(Math.abs(Math.floor(spring.getCurrentValue()) - scrollbars.getScrollTop()) < 1){
    }else{
